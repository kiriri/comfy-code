...
async function run() {
  ...
  for (let key in res) {
    const v = res[key];
    const clean_key = key.replace(/[\s\\\/"']/g, "").replace("+", "Plus").replace(/[(:]/g, "_").replace(")", "");
    let full_path = output_path;
    if (v.category) {
      let path2 = v.category.split("/");
      for (let i = 0; i < path2.length; i++) {
        let partial_path = output_path + path2.slice(0, i + 1).join("/");
        ensure_directory(partial_path);
      }
      full_path = output_path + path2.join("/");
    }
    full_path = import_path.default.join(full_path, clean_key + ".ts");
    ...
    try {
      ...
      import_fs.default.writeFileSync(full_path, `
